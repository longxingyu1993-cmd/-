...
export const StaggeredMenu = ({
  ...
  const animateText = useCallback((opening) => {
    ...
    textCycleAnimRef.current = gsap.to(inner, { yPercent: -finalShift, duration: 0.78, ease: 'power4.out' });
  }, []);
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(colors && colors.length ? colors.slice(0, 3) : ['#94ff27', '#b6ff9f']).map((color, index) => (
          <div key={index} className="sm-prelayer" style={{ background: color }} />
        ))}
      </div>
      <header className="staggered-menu-header" aria-label="移动端导航">
        <div className="sm-logo" aria-label="Logo">
          {logoUrl ? <img src={logoUrl} alt="Logo" className="sm-logo-img" draggable={false} width={110} height={24} /> : <span className="sm-logo-text">LXY</span>}
        </div>
        <button ref={toggleBtnRef} className="sm-toggle" aria-label={open ? '关闭导航菜单' : '打开导航菜单'} aria-expanded={open} aria-controls="staggered-menu-panel" onClick={toggleMenu} type="button">
