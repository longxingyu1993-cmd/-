  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      resize() {
        ...
        const renderScale = rect.width < 768 ? 0.45 : 0.55;
        this.width = Math.max(1, Math.min(1280, Math.floor(rect.width * renderScale)));
        this.height = Math.max(1, Math.min(1600, Math.floor(rect.height * renderScale)));
        this.aspect = this.width / this.height;
        if (this.renderer) this.renderer.setSize(this.width, this.height, false);
      }
      update() {
        const now = performance.now();
        this.delta = Math.min((now - this.lastUpdateTime) / 1000, 0.05);
    ...
    class MouseClass {
      constructor() {
        ...
        this.rawDiff = new THREE.Vector2();
        this.smoothDiff = new THREE.Vector2();
        this.timer = null;
        this.container = null;
        this.docTarget = null;
      ...
      setCoords(x, y) {
        ...
        const nextX = nx * 2 - 1;
        const nextY = -(ny * 2 - 1);
        if (!this.hasUserControl && !this.mouseMoved) {
          this.coords_old.set(nextX, nextY);
          this.smoothDiff.set(0, 0);
        }
        this.coords.set(nextX, nextY);
      ...
      update() {
        ...
        this.rawDiff.subVectors(this.coords, this.coords_old);
        this.coords_old.copy(this.coords);
        if (this.coords_old.x === 0 && this.coords_old.y === 0) this.rawDiff.set(0, 0);
        if (this.rawDiff.lengthSq() > 0.0000001) {
          this.smoothDiff.lerp(this.rawDiff, 0.58);
        } else {
          this.smoothDiff.multiplyScalar(this.isHoverInside ? 0.88 : 0.72);
        }
        this.diff.copy(this.smoothDiff);
...
`;
    ...
    class WebGLManager {
      constructor(props) {
        ...
        this.frameInterval = props.frameInterval || 1000 / 30;
